...
class ResUsersInherit(models.Model):
    ...
    @api.model
    def fields_view_get(
        self, view_id=None, view_type="kanban", toolbar=False, submenu=False
    ):
        ...
        for each in self.search([]):
            ...
            no_of_days = ir_values.get_default(
                "project.config.settings", "no_of_days"
            )
            no_of_hrs = ir_values.get_default(
                "project.config.settings", "working_hr"
            )
            if no_of_days:
                to_date = datetime.today() + timedelta(days=no_of_days)
            else:
                to_date = datetime.today() + timedelta(days=6)
            obj1 = self.env["project.task"].search(
                [
                    ("user_id", "=", each.id),
            ...
            each.write({"maximum_rate": 100, "progress_rate": workload_perc})
        return ret_val
...
class ProjectSettings(models.TransientModel):
    ...
    block_busy_users = fields.Boolean(
        string="Block busy users ?", default=False
    )
...
class ProjectInherit(models.Model):
    ...
    def validation(self):
        ir_values = self.env["ir.values"]
        block_users = ir_values.get_default(
            "project.config.settings", "block_busy_users"
        )
        if block_users:
            if self.user_id.progress_rate > 80:
                raise Warning(
                    _("%s is %s percentage Overloaded with Work")
                    % (self.user_id.name, self.user_id.progress_rate)
                )
...
class EmployeeWorkloadReport(models.TransientModel):
    ...
    working_hr = fields.Integer(
        string="Working Hr/day", required=True, default=8
    )
    ...
    to_date = fields.Date(
        string="To Date",
        required=True,
        default=datetime.today() + timedelta(days=6),
    )

    def workload_report(self):
        data = self.read()[0]
        datas = {"ids": [], "model": "wizard.workload.report", "form": data}
        return self.env["report"].get_action(
            self, "workload_in_project.report_employee_workload", data=datas
        )
