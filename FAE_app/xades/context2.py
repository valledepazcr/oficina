# Library to sign Xades-EPES xml doc
# Based on Tobella's original Xades implementation
# Improved by Ricardo Vong, 2019
...
def create_xades_epes_signature(sign_date=datetime.datetime.now(pytz.timezone('UTC'))):
    ...
    ref = xmlsig.template.add_reference(signature, xmlsig.constants.TransformSha256, 'ReferenceKeyInfo', uri='#' + key_info_id)
    xmlsig.template.add_transform(ref, xmlsig.constants.TransformInclC14N)

    # Reference to the SignedProperties Digest
    ref = xmlsig.template.add_reference(signature, xmlsig.constants.TransformSha256, uri='#' + signed_properties_id, uri_type='http://uri.etsi.org/01903#SignedProperties')
    xmlsig.template.add_transform(ref, xmlsig.constants.TransformInclC14N)
...
class PolicyId2(PolicyId):
    ...
    cache = {
        'https://www.hacienda.go.cr/ATV/ComprobanteElectronico/docs/esquemas/2016/v4.2/ResolucionComprobantesElectronicosDGT-R-48-2016_4.2.pdf': {
